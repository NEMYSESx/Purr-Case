...
const DashBoard = async () => {
  ...
  const orders = db.order.findMany({
    where: {
      isPaid: true,
      createdAt: {
        gte: new Date(new Date().setDate(new Date().getDate() - 7)), //get me orders from the last week
      },
    },
    orderBy: {
      createdAt: "desc",
  ...
  const lastMonthSum = await db.order.aggregate({
    where: {
      isPaid: true,
      createdAt: {
        gte: new Date(new Date().setDate(new Date().getDate() - 30)), //get me orders from the last week
      },
    },
    _sum: {
      amount: true,
